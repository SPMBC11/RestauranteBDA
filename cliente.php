...
<head>
  ...
  <title><?php echo htmlspecialchars($parametros['app_name'], ENT_QUOTES, 'UTF-8'); ?> · Cliente</title>
...
<body>
  ...
  <header>
    <div class="brand">
      ...
      <div>
        <h1><?php echo htmlspecialchars($parametros['app_name'], ENT_QUOTES, 'UTF-8'); ?> · Área Cliente</h1>
        <p class="tagline">Reserva rápida y seguimiento de estado.</p>
      </div>
    </div>
    <div class="user-chip">
      <div>
    ...
    <section>
      <div class="section-head">
        <div>
          ...
          <p class="section-subtitle">Envía tu solicitud; el maitre confirmará.</p>
        </div>
      ...
      <form id="form-reserva" class="form-grid">
        ...
        <label class="form-control">Notas
          <textarea name="notas" placeholder="Cumpleaños, alergias..."></textarea>
        </label>
    ...
    <section>
      <div class="section-head">
        <div>
          ...
          <p class="section-subtitle">Estado en tiempo real.</p>
        </div>
  ...
  <script>
    ...
    async function sendLog(action, meta = '', section = 'Cliente') {
      try {
    ...
    async function loadReservas() {
      ...
      try {
        ...
        if (json.items.length === 0) {
          const li = document.createElement('li');
          li.textContent = 'No tienes reservas aún.';
          list.appendChild(li);
          return;
        }
    ...
    form.addEventListener('submit', async ev => {
      ...
      try {
        ...
        await sendLog('Reserva solicitada', `${data.nombre} · ${data.fecha}`);
        form.reset();
        loadReservas();
      } catch (err) {
